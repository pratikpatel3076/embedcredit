...
async function seedConsumerData() {
  ...
  await ConsentRecord.deleteMany({ userId: userIdKey });
  await ConsentRecord.insertMany([
    {
      id: "CNS-001",
      userId: userIdKey,
      consentType: "AA_FINANCIAL_DATA",
      purpose: "Bank statement analysis, income verification, and cashflow assessment",
      dataCategory: "FINANCIAL_DATA",
      requestedBy: "CreditSaison Prime Line",
      provider: "Finvu Account Aggregator (MOCK)",
      status: "GRANTED",
      ...
      metadata: { scope: ["BANK_ACCOUNTS", "TRANSACTION_HISTORY"] },
    },
    {
      id: "CNS-002",
      userId: userIdKey,
      consentType: "KYC_IDENTITY",
      purpose: "Verified PAN and identity checks (Zero-Aadhaar compliance)",
      dataCategory: "PERSONAL_IDENTIFIERS",
      requestedBy: "PLATFORM",
      provider: "NSDL / PAN Verification System",
      status: "GRANTED",
      consentVersion: "AA-CONSENT-v2.1",
      version: "AA-CONSENT-v2.1",
      grantedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000),
      expiresAt: exp1,
      source: "CONSUMER_ONBOARDING",
    ...
    },
    {
      ...
      requestedBy: "Vantage Credit Engine",
    ...
    },
    {
      ...
      purpose: "Underwriting dossier sharing with selected lending partner",
      dataCategory: "LENDER_PROCESSING",
      requestedBy: "CreditSaison",
      provider: "Vantage Credit Rails",
      status: "GRANTED",
      consentVersion: "AA-CONSENT-v2.1",
      version: "AA-CONSENT-v2.1",
      grantedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
      expiresAt: exp1,
      source: "OFFER_SELECTION_FLOW",
      metadata: { lenderId: "L001" },
    },
    {
      id: "CNS-005",
      userId: userIdKey,
      consentType: "LOAN_SERVICING",
      purpose: "Mandate orchestration and repayment schedule tracking",
      dataCategory: "DLA_PROCESSING",
      requestedBy: "DLA-001",
      provider: "NPCI / Vantage Servicing Rails",
      status: "GRANTED",
      consentVersion: "AA-CONSENT-v2.1",
      version: "AA-CONSENT-v2.1",
      grantedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
      expiresAt: exp1,
      source: "LOAN_DRAWDOWN_FLOW",
      metadata: { channel: "CONSUMER_DASHBOARD" },
    },
  ]);
